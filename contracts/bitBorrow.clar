...
;; define APR
(define-data-var apr unit u10)
(define-data-var timelock unit u0 )
(define-data-var principalvalue unit u0 )
(define-data-var percentage unit u0 )
(define-data-var interest unit u0 )
(define-data-var total unit u0 )
...
(define-public (compound (amount uint) (months uint))
    (begin
    ;; here you use `let` but syntax is incorrect and you didn't created any intermediate variable so it is useless
    ;; we will remove it

    ;; next thing when you write something like this (blablabla) `blablabla` is treated as a function so clarinet is trying to find function `months`
    ;; but it can't find it, because it is a function arbument, so we will remove parenthasis around it

    ;; and we have extra opening paren - remove it
        (var-set timelock months)
    ;; same thing here - pointless parenthasis around `amount`
        (var-set principalvalue amount)

    ;; here you have incorrect syntax for division. It should be: (/ x b)
    ;; and also you dont have parenthasis around var-get - let's fix it
    ;; now it is much  better
        (var-set percentage (/ (var-get apr) u100)
    ;; here you have missing opening parenthasis right before `var-set`
    ;; and as we can see it we don't have closing one so we need to add it as well
    ;; once again mising parenthasis around `var-get` 2x
    
    ;; here I gues you want to calucalte something like: interes = principalvalue * (percentage + 1) * timelok
    ;; something likt this right?

    ;; oh, and if values are defined as uint, 1 should be changed to u1
        (var-set interest (* (var-get principalvalue) (+ (var-get percentage) u1) (var-get timelock) ))
        ;; and here we also have missing parenthasis
        (ok (var-get interest))
    )
)
